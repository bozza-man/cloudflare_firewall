...
async function generateToken() {
  ...
  try {
    // First, let's get the permission group IDs we need
    const permissionsResponse = await axios.get('https://api.cloudflare.com/client/v4/user/tokens/permission_groups', {
      headers: {
        'X-Auth-Email': email,
    ...
    // Find the Zero Trust permission groups
    const permissionGroups = permissionsResponse.data.result;
    
    // Look for Gateway permissions
    const gatewayReadPermission = permissionGroups.find((pg: any) => 
      pg.name === 'Zero Trust:Gateway' || 
      pg.name === 'Account:Zero Trust:Gateway' ||
      pg.name.includes('Gateway') && pg.name.includes('Read')
    );

    const gatewayWritePermission = permissionGroups.find((pg: any) => 
      pg.name === 'Zero Trust:Gateway' || 
      pg.name === 'Account:Zero Trust:Gateway' ||
      pg.name.includes('Gateway') && pg.name.includes('Write') ||
      pg.name.includes('Gateway') && pg.name.includes('Edit')
    );

    if (!gatewayReadPermission && !gatewayWritePermission) {
      spinner.fail('Could not find Zero Trust Gateway permissions');
      console.log('\nAvailable permission groups:');
      permissionGroups
        .filter((pg: any) => pg.name.includes('Zero Trust') || pg.name.includes('Gateway'))
        .forEach((pg: any) => console.log(`  - ${pg.name} (${pg.id})`));
      process.exit(1);
    }
      ...
      policies: [
        {
          ...
          permission_groups: [
            ...(gatewayReadPermission ? [{ id: gatewayReadPermission.id }] : []),
            ...(gatewayWritePermission ? [{ id: gatewayWritePermission.id }] : [])
          ].filter((pg, index, self) => 
    ...
    spinner.text = 'Creating API token...';
    ...
    try {
      ...
      spinner.warn('Token created but could not verify (this might be normal if you have no rules yet)');
    }
